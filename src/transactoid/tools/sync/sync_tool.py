...
class SyncTool:
    ...
    def _mutate_batch_to_derived(
        ...
        for plaid_id in plaid_ids:
            ...
            # drop data the user manually validated. Log a WARNING so the operator
            # knows to opt in explicitly when PR #15 lands.
            if old_derived and any(row.is_verified for row in old_derived):
                ...
                logger.bind(
                    ...
                    "row(s) would be destroyed. Run "
                    "`transactoid re-derive --account-id <id> --include-verified` "
                    "to opt in (future PR #15).",
                    plaid_id,
                    verified_count,
                )
