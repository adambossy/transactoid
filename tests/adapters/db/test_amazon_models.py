...
def create_test_db_with_profile(tmp_path: Path) -> tuple[DB, int]:
    """Create a test database and return it alongside a default profile_id."""
...
class TestAmazonOrderDB:
    ...
    def test_upsert_amazon_order_creates_new_order(self, tmp_path: Path) -> None:
        db, profile_id = create_test_db_with_profile(tmp_path)
    ...
    def test_upsert_amazon_order_reattributes_on_re_scrape(
        self, tmp_path: Path
    ) -> None:
        db, primary_id = create_test_db_with_profile(tmp_path)
        secondary = db.create_amazon_login_profile(
            profile_key="secondary", display_name="Secondary"
        )
        order_id = "112-5793878-2607402"
        db.upsert_amazon_order(
            order_id=order_id,
        ...
        updated = db.upsert_amazon_order(
            order_id=order_id,
            order_date=date(2024, 1, 15),
            order_total_cents=3927,
            profile_id=secondary.profile_id,
        )

        assert updated.profile_id == secondary.profile_id
    ...
    def test_list_amazon_orders_filters_by_profile_id(self, tmp_path: Path) -> None:
        db, primary_id = create_test_db_with_profile(tmp_path)
        secondary = db.create_amazon_login_profile(
            profile_key="secondary", display_name="Secondary"
        )
        ...
        primary_orders = db.list_amazon_orders(profile_id=primary_id)
        secondary_orders = db.list_amazon_orders(profile_id=secondary.profile_id)

        assert {o.order_id for o in primary_orders} == {"primary-order"}
        assert {o.order_id for o in secondary_orders} == {"secondary-order"}
...
class TestAmazonLoginProfileDB:
    ...
    def test_delete_amazon_login_profile_with_orders_raises(
        self, tmp_path: Path
    ) -> None:
        # Opt into SQLite FK enforcement so RESTRICT fires like Postgres would.
        db_path = tmp_path / "test.db"
        db = DB(f"sqlite:///{db_path}", enforce_sqlite_fks=True)
        db.create_schema()
        profile = db.create_amazon_login_profile(
            profile_key="primary", display_name="Primary"
        )
        ...
        with pytest.raises(ValueError, match="2 attributed orders"):
            db.delete_amazon_login_profile(profile_key="primary")
    ...
    def test_set_amazon_profile_history_watermark_sets_date(
        self, tmp_path: Path
    ) -> None:
        db = create_test_db(tmp_path)
        profile = db.create_amazon_login_profile(
        ...
        db.set_amazon_profile_history_watermark(
            profile_id=profile.profile_id, through_date=date(2026, 5, 10)
        )

        refreshed = db.get_amazon_login_profile(profile_key="primary")
        assert refreshed is not None
        assert refreshed.history_complete_through == date(2026, 5, 10)
        assert refreshed.updated_at >= prior_updated_at

    def test_set_amazon_profile_history_watermark_overwrites_prior_value(
        self, tmp_path: Path
    ) -> None:
        db = create_test_db(tmp_path)
        profile = db.create_amazon_login_profile(
        ...
        db.set_amazon_profile_history_watermark(
            profile_id=profile.profile_id, through_date=date(2025, 1, 1)
        )
        db.set_amazon_profile_history_watermark(
            profile_id=profile.profile_id, through_date=date(2026, 5, 10)
        )
    ...
    def test_set_amazon_profile_history_watermark_unknown_profile_raises(
        self, tmp_path: Path
    ) -> None:
        db = create_test_db(tmp_path)
        with pytest.raises(ValueError, match="profile_id=999"):
            db.set_amazon_profile_history_watermark(
                profile_id=999, through_date=date(2026, 5, 10)
            )
