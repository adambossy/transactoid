...
def create_db(tmp_path: Path) -> DB:
    ...
    db = DB(f"sqlite:///{tmp_path / 're_derive_svc.db'}")
    db.create_schema()
    return db
...
class _StubRunner:
    """No-op stub that returns a configurable set of new derived IDs."""
...
def test_re_derive_raises_for_unknown_account(tmp_path: Path) -> None:
    # input
...
def test_re_derive_raises_when_no_sign_convention_configured(tmp_path: Path) -> None:
    # input
...
def test_re_derive_deletes_unverified_rows(tmp_path: Path) -> None:
    # input
    account_id = "acct-rd1"
    ...
    # Old unverified row should be gone; new stub row should be present
...
def test_re_derive_preserves_verified_rows(tmp_path: Path) -> None:
    ...
    # act — use a stub that returns no new rows (simulates mutation with
    # verified-row guard already preventing replacement)
    class _NullRunner:
        def _mutate_batch_to_derived(self, plaid_ids: list[int]) -> list[int]:
            return []
