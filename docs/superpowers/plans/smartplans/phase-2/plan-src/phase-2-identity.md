...
- Each invited spouse's first Google sign-in links to their own existing profile automatically, with no manual setup.
- Anyone whose email is not on the invited list is refused, even with a valid Google login.
- A spouse who later changes their email keeps access to their own data and no one else's.
