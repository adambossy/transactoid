...
# --- Fixed identifiers for the Penny Neon project ---------------------------
ORG_ID="org-sweet-sky-03842625"
PROJECT_ID="purple-poetry-32142000"   # "Penny"
PROD_BRANCH="production"               # the branch we fork FROM (never delete)
TEST_PREFIX="test-"                    # only branches with this prefix are ours
DB_NAME="neondb"
...
# backend/.env.test — GITIGNORED. Generated by scripts/new_test_branch.sh.
# Points the dev/test server at a FRESH one-off Neon branch forked from
# production. Regenerate any time with: backend/scripts/new_test_branch.sh
#
# Branch:      $NEW_BRANCH
# Branch ID:   $NEW_BRANCH_ID
...
echo
echo "   To use it (from backend/):"
echo "     set -a && source .env.test && set +a"
echo "     uv run uvicorn penny.api.main:app --host 127.0.0.1 --port 8000 --reload"
