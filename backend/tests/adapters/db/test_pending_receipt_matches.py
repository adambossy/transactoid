...
def _insert_email_receipt(message_id: str) -> None:
...
def _insert_pending_match(message_id: str, candidate_txn_id: int) -> int:
...
def _count_pending_matches(pending_id: int) -> int:
...
def test_pending_receipt_match_cascade_delete(
    ...
    # setup
    plaid_txn_id = _insert_plaid_txn()
    txn_id = _insert_derived_txn(plaid_txn_id)
    _insert_email_receipt(message_id)
    pending_id = _insert_pending_match(message_id, txn_id)
    ...
    # assert
    assert _count_pending_matches(pending_id) == expected_count
