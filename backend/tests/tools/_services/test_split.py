...
from penny.tools._services.split import split_transaction
...
def _fetch_derived_by_id(transaction_id: int) -> DerivedTransaction | None:
...
def _count_derived_for_plaid(plaid_transaction_id: int) -> int:
...
def test_split_transaction_two_way(isolated_db: None) -> None:
    # input
    plaid_id = _insert_plaid_txn(amount_cents=5000)
    txn_id = _insert_derived_txn(plaid_id, amount_cents=5000)
    parts = [3000, 2000]

    # act
    new_ids = split_transaction(get_db(), txn_id, parts)

    # expected: two new rows with correct amounts/provenance
    assert len(new_ids) == 2
    row0 = _fetch_derived_by_id(new_ids[0])
    row1 = _fetch_derived_by_id(new_ids[1])
    ...
    all_derived = _count_derived_for_plaid(plaid_id)
    assert all_derived == 2


def test_split_transaction_three_way(isolated_db: None) -> None:
    # input
    plaid_id = _insert_plaid_txn(external_id="ext-002", amount_cents=9000)
    txn_id = _insert_derived_txn(plaid_id, external_id="derived-002", amount_cents=9000)
    ...
    rows = [_fetch_derived_by_id(nid) for nid in new_ids]
...
def test_split_transaction_not_found(isolated_db: None) -> None:
    with pytest.raises(SplitError, match="transaction 9999 not found"):
        split_transaction(get_db(), 9999, [2500, 2500])


def test_split_transaction_verified_rejection(isolated_db: None) -> None:
    # input
    plaid_id = _insert_plaid_txn(external_id="ext-003", amount_cents=5000)
    txn_id = _insert_derived_txn(
        plaid_id, external_id="derived-003", amount_cents=5000, is_verified=True
    )

    # act + assert
    with pytest.raises(SplitError, match="is verified and cannot be modified"):
        split_transaction(get_db(), txn_id, [3000, 2000])


def test_split_transaction_amazon_gated_rejection(isolated_db: None) -> None:
    ...
    txn_id = _insert_derived_txn(
        plaid_id, external_id="derived-amazon-001", amount_cents=5000
    )
...
def test_split_transaction_amount_mismatch_rejection(isolated_db: None) -> None:
    plaid_id = _insert_plaid_txn(external_id="ext-004", amount_cents=5000)
    txn_id = _insert_derived_txn(plaid_id, external_id="derived-004", amount_cents=5000)

    with pytest.raises(SplitError, match="they must sum exactly"):
        split_transaction(get_db(), txn_id, [3000, 1000])  # sums to 4000, not 5000


def test_split_transaction_single_part_rejection(isolated_db: None) -> None:
    plaid_id = _insert_plaid_txn(external_id="ext-005", amount_cents=5000)
    txn_id = _insert_derived_txn(plaid_id, external_id="derived-005", amount_cents=5000)

    with pytest.raises(SplitError, match="at least 2 parts"):
        split_transaction(get_db(), txn_id, [5000])


def test_split_transaction_zero_part_rejection(isolated_db: None) -> None:
    plaid_id = _insert_plaid_txn(external_id="ext-006", amount_cents=5000)
    txn_id = _insert_derived_txn(plaid_id, external_id="derived-006", amount_cents=5000)

    with pytest.raises(SplitError, match="all parts must be > 0"):
        split_transaction(get_db(), txn_id, [5000, 0])


def test_split_transaction_negative_part_rejection(isolated_db: None) -> None:
    plaid_id = _insert_plaid_txn(external_id="ext-007", amount_cents=5000)
    txn_id = _insert_derived_txn(plaid_id, external_id="derived-007", amount_cents=5000)

    with pytest.raises(SplitError, match="all parts must be > 0"):
        split_transaction(get_db(), txn_id, [6000, -1000])
...
def test_split_transaction_atomicity(
    ...
    db = get_db()
    plaid_id = _insert_plaid_txn(external_id="ext-008", amount_cents=5000)
    txn_id = _insert_derived_txn(plaid_id, external_id="derived-008", amount_cents=5000)
    ...
    surviving = _fetch_derived_by_id(txn_id)
