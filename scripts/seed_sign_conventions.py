...
from transactoid.adapters.db.sign_convention_defaults import (  # noqa: E402
...
def seed_sign_conventions(*, dry_run: bool = False) -> dict[str, int]:
    ...
    if not dry_run:
        print(
            "NEXT STEP: run `scripts/backfill_sign_conventions.py` to normalize "
            "historical rows for expense_negative accounts."
        )
    return counts
